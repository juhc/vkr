...
- использовать **Ansible Vault** (опционально), если вам удобно шифровать файл:

```bash
ansible-vault create stands/<scenario>/infrastructure/ansible/group_vars/all/accounts.yml
```

или хранить `accounts.yml` локально (вне git) и передавать через `-e @...`/CI variables / менеджер секретов.
