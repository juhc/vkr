...
Здесь лежат **Packer-шаблоны Linux** для `scenario-linux-stand`.  
Windows-шаблоны находятся отдельно в `scenario-windows-stand`.
...
```powershell
cd D:\zxc\vkr\stands\scenario-linux-stand\infrastructure\templates
...
```bash
cd stands/scenario-linux-stand/infrastructure/templates
