...
- линкует все GPO вида `VKR-ad.gpo.*` к OU стенда,
...
- `vkr_ad_org_enabled` (bool, default `true`)
- `vkr_ad_training_ou` (string, default `Training`)
- `vkr_ad_stand_ou` (string, default `Stand-01`)
- `vkr_ad_stand_id` (string, default `stand-01`) — используется для имени группы
- `vkr_ad_stand_computers` (list[string]) — список имён компьютерных аккаунтов (COMPUTERNAME) в домене
- `vkr_ad_gpo_prefix` (string, default `VKR-ad.gpo.`)
