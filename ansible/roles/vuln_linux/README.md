...
- `vuln_linux_weak_password` (string, default `password`): пароль для слабых учёток (хэшируется).
- `vuln_linux_extra_admin_user` (string, default `testadmin`): имя лишнего admin/sudo пользователя.
...
2) Добавьте ID в `defaults/main.yml` → `vuln_linux_known`.
