...
- `vkr_vuln_ad_domain_name` (string, default `training.local`): DNS‑имя домена (best-effort).
- `vkr_vuln_ad_weak_password` (string, default `password`): “слабый” пароль для демонстрации (vault не нужен, но можно).
- `vkr_vuln_ad_test_user` (string, default `StudentTest`): имя тестового доменного пользователя.
...
2) Добавьте ID в `defaults/main.yml` → `vkr_vuln_ad_known`.
