...
Идея: в сценарии вы задаёте `accounts_profiles.<group>` в `group_vars/all/accounts.yml`, а playbook передаёт список пользователей/групп в роль `accounts`.

## Переменные

- `accounts_groups` (list[str]): группы, которые надо создать (Linux).
- `accounts_users` (list[dict]): пользователи.

Формат `accounts_users`:
