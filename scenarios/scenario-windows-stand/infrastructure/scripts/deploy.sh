...
По умолчанию выполняет всё:
  ...
  2) Terraform apply (windows-ws + windows-server + domain-controller)
...
if [[ $DO_TERRAFORM -eq 1 ]]; then
  ...
  for comp in windows-ws windows-server domain-controller; do
...
if [[ $DO_ANSIBLE -eq 1 ]]; then
  ...
  echo "-- windows-ws playbook"
  (cd "$ANS_DIR" && ansible-playbook windows-ws/playbook.yml)
