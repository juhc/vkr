...
```
  ...
  (192.168.10.0/24) (192.168.20.0/24) (192.168.30.0/24) (192.168.50.0/24)
    ...
    +---+---+         +---+---+         +---+---+---+       +---+---+
    |       |         |       |         |       |   |       |       |
  RADIUS          Billing  Web      Monitoring  Jump Admin  Kali Linux
  Server          Server Server     Server      Server Workstation Pentest Box
```

### Сетевые сегменты

Сценарий упрощен для удобства обучения и содержит **4 ключевых машины инфраструктуры** (плюс машина атакующего):
...
**Устройства**:
- **Сервер сервисов** (192.168.20.10)
  - ОС: Ubuntu 20.04 LTS
  - Роль: Биллинговая система + веб-сайт (объединенный)
  - Сервисы: Apache, PHP, PostgreSQL, WordPress
  - Уязвимости: Слабые пароли БД, SQL инъекции, устаревший WordPress, XSS, необновленное ПО, утечка данных клиентов
...
**Устройства**:
- **Сервер управления** (192.168.30.10)
  - ОС: Ubuntu 20.04 LTS
  - Роль: Мониторинг + Jump-сервер (объединенный)
  - Сервисы: Zabbix, SNMP, SSH, OpenVPN
  - Уязвимости: Слабые SNMP community strings, открытый веб-интерфейс, доступ по паролю SSH, отсутствие 2FA, слабые пароли, отсутствие защиты от брутфорса, раскрытие информации

- **Компьютер администратора** (192.168.30.20)
  - ОС: Ubuntu 20.04 Desktop
  - Роль: Рабочая станция администратора
  - Сервисы: SSH, RDP, SMB, FTP
  - Уязвимости: Слабые пароли, открытые сетевые папки, устаревшее ПО, отсутствие антивируса, сохраненные учетные данные в браузере
...
ssh platform-admin@192.168.30.10  # Сервер управления (мониторинг + jump)
ssh platform-admin@192.168.30.20  # Компьютер администратора
```
