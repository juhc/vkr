...
```
  ...
  (192.168.10.0/24)          (192.168.20.0/24)          (192.168.30.0/24)          (192.168.50.0/24)
    ...
    +---+---+                   +---+---+                   +---+---+                   +---+---+
    |       |                   |       |                   |       |                   |       |
  RADIUS                  Billing  Web                  Monitor  Jump                  Kali Linux
...
| IP адрес | Имя хоста | Роль | ОС | Сервисы | Открытые порты | Уязвимости |
|----------|-----------|------|----|---------|----------------|------------|
| 192.168.20.10 | billing-server | Биллинговый сервер | Ubuntu 20.04 | Apache, PHP, PostgreSQL | 80, 443, 5432 | Слабые пароли БД, SQL инъекции, необновленное ПО, утечка данных клиентов |
| 192.168.20.20 | web-server | Веб-сервер | Ubuntu 20.04 | Apache, PHP, WordPress | 80, 443, 8080 | Устаревшая версия WordPress, SQL инъекции, XSS, небезопасная конфигурация Apache |
...
| IP адрес | Имя хоста | Роль | ОС | Сервисы | Открытые порты | Уязвимости |
|----------|-----------|------|----|---------|----------------|------------|
| 192.168.30.10 | monitoring-server | Сервер мониторинга | Ubuntu 20.04 | Zabbix, SNMP | 80, 443, 10051, 161 | Слабые SNMP community strings, открытый веб-интерфейс, слабые пароли, раскрытие информации |
| 192.168.30.20 | jump-server | Jump-сервер | Ubuntu 20.04 | SSH, OpenVPN | 22, 1194 | Доступ по паролю, отсутствие 2FA, слабые пароли, отсутствие защиты от брутфорса |
