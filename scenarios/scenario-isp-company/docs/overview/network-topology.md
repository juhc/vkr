...
|----------|-----------|------|----|---------|----------------|------------|
| 192.168.10.10 | radius-server | RADIUS сервер | Ubuntu 20.04 | FreeRADIUS, MySQL | 1812, 1813, 3306 | Слабый пароль БД, небезопасная конфигурация, утечка данных клиентов |
...
| IP адрес | Имя хоста | Роль | ОС | Сервисы | Открытые порты | Уязвимости |
|----------|-----------|------|----|---------|----------------|------------|
| 192.168.20.10 | services-server | Сервер сервисов (Биллинг + Веб) | Ubuntu 20.04 | Apache, PHP, PostgreSQL, WordPress | 80, 443, 5432, 8080 | Слабые пароли БД, SQL инъекции, устаревший WordPress, XSS, необновленное ПО, утечка данных клиентов |
...
| IP адрес | Имя хоста | Роль | ОС | Сервисы | Открытые порты | Уязвимости |
|----------|-----------|------|----|---------|----------------|------------|
| 192.168.30.10 | management-server | Сервер управления (Мониторинг + Jump) | Ubuntu 20.04 | Zabbix, SNMP, SSH, OpenVPN | 80, 443, 10051, 161, 22, 1194 | Слабые SNMP community strings, открытый веб-интерфейс, доступ по паролю SSH, отсутствие 2FA, слабые пароли, отсутствие защиты от брутфорса, раскрытие информации |
| 192.168.30.20 | admin-workstation | Компьютер администратора | Ubuntu 20.04 Desktop | SSH, RDP, SMB, FTP | 22, 3389, 445, 21 | Слабые пароли, открытые сетевые папки, устаревшее ПО, отсутствие антивируса, сохраненные учетные данные в браузере |
