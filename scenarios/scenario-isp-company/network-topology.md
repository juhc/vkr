...
```
  ...
  (10.0.0.0/8)            (192.168.10.0/24)          (192.168.20.0/24)          (192.168.30.0/24)          (192.168.50.0/24)
    ...
    +---+---+                   +---+---+                   +---+---+                   +---+---+                   +---+---+
    |       |                   |       |                   |       |                   |       |                   |       |
  PPPoE  DHCP              RADIUS  DNS  NTP            Billing Web Mail CRM        Monitor Jump Backup          Kali Linux
  Server Server             Server Server Server        Server Server Server Server Server Server Server        Pentest Box
```
...
- **Сканирование и разведка**: Nmap, Masscan, Zmap, Netdiscover
- **Эксплуатация**: Metasploit Framework, ExploitDB, SearchSploit
- **Веб-тестирование**: Burp Suite, OWASP ZAP, SQLMap, XSSer
- **Брутфорс**: Hydra, Medusa, John the Ripper, Hashcat
- **Анализ трафика**: Wireshark, Tcpdump, Ettercap
- **Анализ уязвимостей**: OpenVAS, Nikto, Nessus (если доступен)
- **Социальная инженерия**: Social Engineering Toolkit (SET)
- **Пост-эксплуатация**: Mimikatz (для Windows), BloodHound (для AD)
- **Другие**: Aircrack-ng, Reaver, Wifite, Aircrack-ng
