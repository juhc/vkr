...
def main():
    ...
    doc.add_heading("1. ТИПОВЫЕ УЯЗВИМОСТИ", level=1)
    doc.add_paragraph(
        "В главе описаны типовые уязвимости и ошибки администрирования, характерные для небольших организаций, а также принципы "
        "их отбора для учебной площадки по курсу «Защита ОС». Основной акцент сделан на конфигурационных ошибках, которые встречаются "
        "в реальных аудитах: слабые политики аутентификации, избыточные привилегии, небезопасные протоколы, отключённые обновления и защита, "
        "а также недостаточное логирование/аудит. Отдельно рассматриваются доменные эффекты (AD/GPO), когда ошибочная политика масштабируется "
        "на множество машин."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        "В контексте данной работы под «типовой уязвимостью» понимается воспроизводимая комбинация небезопасных настроек, "
        "которая повышает риск компрометации и может быть проверена объективными артефактами (командами, политиками, логами). "
        "Под «ошибкой администрирования» понимается действие/решение (часто мотивированное удобством или недостатком компетенций), "
        "которое приводит к такой небезопасной конфигурации (например, отключение обновлений ради стабильности или ослабление аудит‑политик)."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        "Для учебных целей важно, чтобы каждая уязвимость: (1) включалась/выключалась контролируемо, (2) имела понятный способ проверки «до/после», "
        "(3) приводила к наблюдаемым последствиям (рост поверхности атаки, упрощение brute-force/relay, ухудшение расследования), "
        "(4) обучала корректному исправлению (hardening), а не только «поиску галочек»."
    )
    ...
    doc.add_paragraph("Краткая характеристика выбранных категорий (что обычно проверяется и чему учит студент):")
    add_bullets(
        doc,
        f"- {category_name('1')}: политика паролей/lockout, слабые учётки; учит проверять password policy и параметры аутентификации.\n"
        f"- {category_name('2')}: лишние админы, ослабление UAC/sudo/SUID; учит анализировать права и пути повышения привилегий.\n"
        f"- {category_name('3')}: SSH/RDP/WinRM и их параметры; учит ограничивать поверхность удалённого доступа и настраивать безопасные режимы.\n"
        f"- {category_name('4')}: права на файлы, секреты, небезопасный PATH; учит находить утечки и локальные векторы эскалации.\n"
        f"- {category_name('5')}: отключение обновлений; учит выстраивать патч‑менеджмент и оценивать риск накопления уязвимостей.\n"
        f"- {category_name('6')}: небезопасные протоколы/службы (SMB1/NTLMv1/LLMNR/Telnet/FTP и т.п.); учит снижать риск MITM/relay и lateral movement.\n"
        f"- {category_name('7')}: firewall и периметр; учит сегментации и минимизации входящих сервисов.\n"
        f"- {category_name('8')}: аудит/логирование/удержание; учит обеспечивать видимость событий и готовность к расследованию.\n"
        f"- {category_name('9')}: базовая защита (Defender/AV); учит оценивать влияние отключения защитных механизмов на устойчивость системы.",
    )
    ...
    doc.add_heading("1.5 Методика выполнения учебной работы (аудит → доказательства → исправление)", level=2)
    doc.add_paragraph(
        "Практикум строится по циклу «аудит → доказательства → исправление → повторная проверка». "
        "Обучающийся получает доступ к стенду, включает ВМ, выполняет аудит конфигураций и фиксирует найденные проблемы "
        "в виде доказательств (выводы команд, фрагменты политик, скриншоты). Затем применяет исправления (hardening) и повторно "
        "проводит проверку, демонстрируя разницу «до/после»."
    )
    add_bullets(
        doc,
        "- шаг 1: инвентаризация (какие ВМ, какие роли, какие сервисы слушают порты);\n"
        "- шаг 2: проверка категорий 1..9 по чек‑листу (аутентификация, права, протоколы, обновления, аудит и т.д.);\n"
        "- шаг 3: формирование гипотез «какая атака возможна» (связь с MITRE ATT&CK);\n"
        "- шаг 4: исправление (настройка политик, отключение протоколов, включение защиты/аудита);\n"
        "- шаг 5: повторная проверка + отчёт с доказательствами «до/после».",
    )
    add_placeholder(doc, "table", "Чек-лист проверки по категориям 1..9 (команды/политики/артефакты)", counters)

    doc.add_heading("1.6 Критерии оценки результата", level=2)
    doc.add_paragraph(
        "Оценивание строится вокруг качества аудита и корректности исправлений, а не только количества найденных пунктов. "
        "Важны доказательства, корректная интерпретация риска и воспроизводимость результата."
    )
    add_bullets(
        doc,
        "- полнота: покрыты все категории, предусмотренные заданием;\n"
        "- доказательность: есть артефакты проверки (вывод команд, политики, логи, скриншоты);\n"
        "- корректность исправлений: изменения устраняют проблему и не ломают базовую работоспособность стенда;\n"
        "- объяснение: студент описывает связь «конфигурация → риск → техника атаки → исправление»;\n"
        "- повторяемость: шаги исправления можно повторить по отчёту на другом экземпляре стенда.",
    )
    ...
    doc.add_paragraph(
        "Проектирование учебной площадки рассматривается как инженерная задача: необходимо обеспечить воспроизводимость, "
        "масштабируемость на группу обучающихся, переносимость между площадками, а также удобство сопровождения каталога уязвимостей. "
        "В данной главе фиксируются принципы и принятые архитектурные решения, чтобы систему можно было развивать и поддерживать "
        "без «ручных шаманств» и зависимости от конкретной сети/Proxmox инсталляции."
    )

    doc.add_heading("2.1 Требования и ключевые проектные решения", level=2)
    doc.add_paragraph(
        "Требования сформированы с учётом практики проведения лабораторных работ: администратор должен быстро подготовить стенды, "
        "обучающиеся — получить ограниченный доступ к своим ВМ, а преподаватель — иметь возможность включать разные наборы уязвимостей "
        "в зависимости от темы занятия. Дополнительно учитывались ограничения небольших организаций: отсутствие выделенного SOC, "
        "частая эксплуатация «как получится», нехватка времени на регулярный hardening и контроль конфигураций."
    )
    add_bullets(
        doc,
    ...
    doc.add_paragraph(
        "Архитектура построена по слоистому принципу (templates → Terraform → Ansible): шаблоны обеспечивают управляемую гостевую ОС "
        "и базовые зависимости (SSH, PowerShell, cloud-init), Terraform отвечает за декларативное создание ВМ и их базовые параметры, "
        "Ansible — за конфигурацию на уровне ОС и домена (учётные записи, join, уязвимости, GPO). "
        "Такое разделение снижает связанность: уязвимости можно менять без пересборки образов, а параметры ресурсов/сети — без правок ролей."
    )
    add_bullets(
        doc,
    ...
    doc.add_paragraph(
        "Структура репозитория построена так, чтобы «код уязвимостей» был единым и переиспользуемым, а стенды отличались в основном "
        "параметрами (tfvars/group_vars) и составом ВМ. Это упрощает расширение: добавление нового стенда не требует копирования "
        "каталога уязвимостей — достаточно подключить общие роли и определить профили."
    )
    add_bullets(
        doc,
    ...
    doc.add_paragraph(
        "Стенд рассматривается как «мини‑инфраструктура» небольшой организации. Важно, что сеть и точки управления являются частью модели: "
        "поэтому сетевые параметры задаются переменными (bridge, DHCP/статик, шлюз, DNS), а доменная интеграция включается профилем. "
        "Это позволяет переносить стенды между площадками и варьировать условия (например, разные подсети для разных групп)."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        "Центральная идея проектирования каталога уязвимостей — «уязвимость как модуль». Модуль имеет ID, понятную проверку и сценарий обучения. "
        "Включение выполняется списком ID, что позволяет собирать разные наборы уязвимостей (профили) под тему занятия без изменения кода."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        "Профиль уязвимостей — это список ID для группы хостов. На практике профиль соответствует «варианту лабораторной работы» "
        "(базовый/продвинутый/экзаменационный). Роль уязвимостей не зависит от стенда: стенд задаёт только список ID, а роль — механизм применения."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        "Расширение каталога должно выполняться по единому стандарту, иначе стенд быстро становится несопровождаемым. "
        "Рекомендуется: (а) выбирать ID по схеме `<scope>.<domain>.<name>` (например linux.ssh.*, windows.network.*, ad.gpo.*), "
        "(б) делать задачи идемпотентными, (в) описывать проверку включения уязвимости и ожидаемый эффект, "
        "(г) не «ломать» хост полностью (уязвимость должна быть исправляемой в рамках занятия)."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        "Доменная среда вводится как обязательная, потому что многие реальные ошибки администрирования проявляются именно на уровне домена: "
        "небезопасные GPO, слабые доменные политики, протоколы, влияющие на весь сегмент. Организационная автоматизация нужна для масштаба: "
        "при большом числе стендов важно, чтобы GPO применялись только к компьютерам конкретного стенда (через OU и security filtering)."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        "Так как Ansible работает по SSH и не требует агентской установки, управляемость стенда полностью определяется качеством шаблонов. "
        "Для Linux важны: корректный cloud-init, SSH доступ, совместимость UEFI/OVMF. Для Windows: включённый OpenSSH Server и возможность "
        "исполнять команды PowerShell через SSH. QEMU Guest Agent рекомендуется для удобства эксплуатации в Proxmox (получение IP, статусы, shutdown)."
    )
    doc.add_paragraph(
    ...
    doc.add_heading("2.11 Модель конфигураций и параметризация (Terraform tfvars + Ansible group_vars)", level=2)
    doc.add_paragraph(
        "Переносимость достигается тем, что все «параметры площадки» вынесены в переменные, а код IaC остаётся неизменным. "
        "Terraform задаёт инфраструктурные параметры (Proxmox API, node/storage, bridge, IP/DHCP, gateway, DNS), а Ansible — параметры домена, "
        "учётных записей и профили уязвимостей. Такое разделение снижает риск ошибок: администратор редактирует небольшое число файлов, "
        "а логика остаётся стабильной."
    )
    add_bullets(
        doc,
        "- Terraform: `terraform.tfvars` (локально) — Proxmox/сеть/шаблон/ресурсы;\n"
        "- Ansible: `group_vars/all/ad.yml` — доменная интеграция + организационные параметры OU/GPO;\n"
        "- Ansible: `group_vars/all/accounts.yml` — профили локальных пользователей/ключей;\n"
        "- Ansible: `group_vars/all/vulnerabilities.yml` — профили уязвимостей (списки ID).",
    )
    add_placeholder(doc, "table", "Таблица: границы ответственности Terraform vs Ansible (какие параметры где задаются)", counters)
    doc.add_page_break()

    # 3) Реализация учебной площадки
    doc.add_heading("3. РЕАЛИЗАЦИЯ УЧЕБНОЙ ПЛОЩАДКИ", level=1)
    doc.add_paragraph(
        "В главе описана практическая реализация учебной площадки: какие компоненты и файлы конфигурации используются, "
        "как организован процесс развертывания стенда и как выполняется сопровождение. "
        "Для удобства эксплуатации материал изложен как «инженерная инструкция», но с привязкой к структуре проекта: "
        "templates/Packer → Terraform → Ansible (accounts → (опц.) ad_join → vuln_*)."
    )
    doc.add_paragraph(
        "Реализация ориентирована на принцип IaC: при изменениях администратор корректирует переменные (tfvars/group_vars) "
        "и повторно применяет автоматизацию. Это снижает долю ручных операций и позволяет воспроизводить стенды для разных групп "
        "обучающихся с минимальными отличиями (IP/bridge/pool/profiles)."
    )
    ...
    doc.add_paragraph(
        "На практике рекомендуется хранить заполненные файлы конфигурации отдельно для каждой группы/потока (например в закрытом хранилище), "
        "а в репозитории держать только `*.example`. Это снижает риск утечки секретов (API token, join password, приватные ключи) и упрощает аудит изменений."
    )
    add_bullets(
        doc,
    ...
    add_placeholder(doc, "table", "Таблица: файлы конфигурации стенда → что задаётся → что является секретом", counters)
    ...
    doc.add_paragraph(
        "Практика показала, что для устойчивого деплоя важно явно выдерживать порядок для Windows-стенда: "
        "сначала настраивается Domain Controller, затем выполняется domain join клиентов, затем DC запускается повторно "
        "для организационной автоматизации (перемещение компьютерных аккаунтов в OU и применение/фильтрация GPO). "
        "Именно этот порядок реализован в deploy-скрипте."
    )
